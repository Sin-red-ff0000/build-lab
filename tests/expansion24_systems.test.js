...
assert.deepEqual({chars:Object.keys(D.CHARACTERS).length,styles:Object.keys(D.CHARACTER_STYLES).length,relics:Object.keys(D.RELICS).length,protocols:Object.keys(D.PROTOCOLS).length,tunings:Object.keys(D.TUNINGS).length,traits:Object.keys(D.TRAITS).length,behaviors:Object.keys(D.ENEMY_BEHAVIORS).length},{chars:35,styles:86,relics:365,protocols:143,tunings:52,traits:81,behaviors:88});
let s=BL.Store.mergeDefaults({version:23,boss4Defeated:true,alchemy:{enabled:true,allocation:{fire:6,earth:6},recipes:['lava'],hold:[]}});assert.equal(s.version,24);assert.equal(s.alchemy.allocation.fire,6);
