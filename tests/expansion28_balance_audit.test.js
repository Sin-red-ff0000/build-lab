...
// v0.56 replaces the old vertical payoff ladder for relic/protocol/tuning systems.
if(D.V56_SYSTEM_AUDIT){
  for(const id of A.adjustedRelics){const c=D.V27_RELIC_RULES[id][0];assert(c.mult<=1.12,id+' v0.56 relic cap');}
  for(const id of A.adjustedProtocols){const r=D.V27_PROTOCOL_RULES[id];assert(r.hit<=1.12&&r.miss>=.88,id+' v0.56 protocol band');}
  for(const id of A.adjustedTunings){const r=D.V27_TUNING_RULES[id];assert(r.hit<=1.12&&r.miss>=.88,id+' v0.56 tuning band');}
}else{
...
assert(D.V27_CARD_RULES.v27_apex_probe[0].mult>=1.70);assert(D.V27_CARD_RULES.v27_complex_probe[0].mult>=1.70);if(D.V56_SYSTEM_AUDIT){assert(D.V27_TUNING_RULES.v27_t_rune_opus.hit<=1.12);assert(D.V27_TUNING_RULES.v27_t_link_arcana.hit<=1.12);}else{assert(D.V27_TUNING_RULES.v27_t_rune_opus.hit>=1.64);assert(D.V27_TUNING_RULES.v27_t_link_arcana.hit>=1.64);}
