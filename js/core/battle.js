...
(function(){
  ...
  // v0.49: 共通イベント基盤。カード/遺物/スタイルが同じ戦闘イベントを参照できるよう、
  // 個別の if 文を増やす前に履歴を一か所へ集約する。
  function ensureEventState(){
    const b=Battle.current;if(!b)return null;
    return b.events||(b.events={seq:0,counts:{},byCard:{},recent:[]});
  }
  function emitEvent(type,payload={}){
    const b=Battle.current,e=ensureEventState();if(!b||!e||!type)return null;
    const ev={id:++e.seq,type,turn:b.turn,...payload};e.counts[type]=(e.counts[type]||0)+1;
    const uid=payload.uid||payload.instance?.uid;if(uid){const c=e.byCard[uid]||(e.byCard[uid]={counts:{},last:null});c.counts[type]=(c.counts[type]||0)+1;c.last=ev;}
    e.recent.push(ev);if(e.recent.length>40)e.recent.shift();
    // v0.51: イベント履歴による変質。カードごとの専用ifではなく共通イベントから判定する。
    if(uid){const inst=allOwnedInstances().find(x=>x.uid===uid);const def=inst&&D.CARDS[inst.transformedCardId||inst.cardId],tr=def?.transformAfterEvent;if(inst&&tr&&tr.type===type&&!inst.transformedCardId&&eventCount(type,uid)>=tr.count){inst.transformedCardId=tr.into;const into=D.CARDS[tr.into];e.counts.transformed=(e.counts.transformed||0)+1;const ce=e.byCard[uid]||(e.byCard[uid]={counts:{},last:null});ce.counts.transformed=(ce.counts.transformed||0)+1;const tev={id:++e.seq,type:'transformed',turn:b.turn,uid,cardId:inst.cardId,into:tr.into};ce.last=tev;e.recent.push(tev);if(e.recent.length>40)e.recent.shift();log(`《${def.name}》が《${into?.name||tr.into}》へ変質。`);}}
    return ev;
  }
  function eventCount(type,uid=null){const e=ensureEventState();if(!e)return 0;return uid?(e.byCard[uid]?.counts?.[type]||0):(e.counts[type]||0);}
  function recentEvents(type=null){const e=ensureEventState();if(!e)return[];return type?e.recent.filter(x=>x.type===type):[...e.recent];}
  function recordHpChange(delta,source){const b=Battle.current;if(!b||!delta)return;const h=b.hpHistory||(b.hpHistory={selfDamageCount:0,selfDamageTotal:0,healCount:0,healTotal:0,enemyDamageTotal:0,halfCrossings:0,recoveries:0,directionSwitches:0,lastDirection:null});const before=b.player.hp,after=Math.max(0,Math.min(b.player.maxHp,before+delta)),dir=delta>0?'up':'down';if(h.lastDirection&&h.lastDirection!==dir)h.directionSwitches++;h.lastDirection=dir;if(source==='self'){h.selfDamageCount++;h.selfDamageTotal+=before-after;}else if(source==='heal'){h.healCount++;h.healTotal+=after-before;}else if(source==='enemy')h.enemyDamageTotal+=before-after;const half=b.player.maxHp/2;if((before>half&&after<=half)||(before<=half&&after>half)){h.halfCrossings++;if(before<=half&&after>half)h.recoveries++;}b.player.hp=after;emitEvent(source==='self'?'selfDamage':source==='heal'?'heal':'enemyDamage',{amount:Math.abs(after-before),before,after});}
  function card(x){let base=D.CARDS[x.transformedCardId||x.cardId];if(D.resolveDualFaceCard)base=D.resolveDualFaceCard(x,Battle.current,state());if(D.resolveMultiStageCard)base=D.resolveMultiStageCard(x,Battle.current,state());if(D.resolveAdaptiveCard)base=D.resolveAdaptiveCard(base,x,Battle.current,state());return base;}
  function pileFindAndRemove(uid){
    const b=Battle.current;if(!b)return null;
    const piles=['draw','discard','excluded','vanished','reserved','prompt','resolving'];
  ...
  function allOwnedInstances(){
    ...
    const arr=[...b.draw,...b.discard,...b.excluded,...b.vanished,...b.reserved,...b.prompt,...b.resolving];if(b.centerReserved)arr.push(b.centerReserved);return arr;
  }
  function invariant(){
  ...
  function statusCount(){const s=Battle.current.enemy.status;return ['poison','burn','bleed','frost','vulnerable','weak'].filter(k=>s[k]>0).length;}
  ...
  function v19RuleMatch(when,ctx){
    ...
    if(when.prevMulti&&!b.lastWasMulti)return false;
    if(when.prevSelfDamage){const pc=b.lastUsed?card(b.lastUsed):null;if(!(pc?.selfDamage|| (b.lastUsed&&conversionFor(b.lastUsed)==='blood_role')))return false;}
    if(when.enemyAtkMin!=null&&Number(b.enemy.atk||0)<when.enemyAtkMin)return false;
    if(when.reshufflesMin!=null&&Number(b.reshuffles||0)<when.reshufflesMin)return false;
    if(when.returnBottom&&!ctx.c.returnBottom)return false;
  ...
  function create(mode=false){
    ...
    Battle.current={
      ...
      player:{hp:startHp,maxHp:startHp,block:0,nextBuff:0,nextPenalty:0,nextSingleHitBuff:0,nextBlockFlat:0,nextStatusFlat:0},
      enemy:{...e,block:0,status:{poison:0,burn:0,bleed:0,frost:0,vulnerable:0,weak:0}},
      draw:U.shuffle(instances),discard:[],excluded:[],vanished:[],prompt:[],reserved:[],resolving:[],centerReserved:null,
      ...
      lastWasAttack:false,lastWasMulti:false,lastWasBlock:false,discardedThisTurn:0,counter:0,fixedCounter:0,usedBlockUid:null,fastAccumulator:0,perfectWallHealedThisEnemyTurn:false,bastionMemoryHealedThisEnemyTurn:false,statusBlockGained:0,lastChoicePosition:null,previousChoicePosition:null,repeatedPosition:false,positionStreak:0,positionHistory:{centerChain:0,sameChain:0,changes:0,sideAlternations:0,lastSide:null},enemyHealed:false,enemyHealedLastTurn:false,behaviorHealedLastTurn:false,behaviorTriggered:{},enemyTempAtk:0,suppressRegen:false,linkComboCount:0,linkForwardCount:0,linkReverseCount:0,linkHistory:{pairStreak:0,directionAlternations:0,lastDirection:null},tuningHistory:{focus:0,reserve:0,relay:0},conversionHistory:{uses:0,switches:0,last:null},v35StyleTriggerCount:0,v35CharacterTriggerCount:0,boss4LastLayer:null,boss4RepeatedLayer:false,
      hpHistory:{selfDamageCount:0,selfDamageTotal:0,healCount:0,healTotal:0,enemyDamageTotal:0,halfCrossings:0,recoveries:0,directionSwitches:0,lastDirection:null},
      events:{seq:0,counts:{},byCard:{},recent:[]},records:{last:null,count:0},
      log:[]
    };
  ...
  function reshuffle(includeDraw=false){
    ...
    b.draw=U.shuffle(pool);b.discard=[];b.excluded=[];b.reshuffles++;b.recentReshuffle=true;b.firstCardAfterReshuffle=true;emitEvent('reshuffle',{count:b.reshuffles});log('山札を再構築。');BL.Alchemy?.supply(b);
  ...
  function beginTurn(){
    ...
    while(b.reserved.length&&b.prompt.length<count){const x=b.reserved.shift();b.prompt.push(x);emitEvent('reservationArrived',{uid:x.uid,cardId:x.cardId,position:'prompt'});}
    ...
    b.prompt.forEach((x,i)=>emitEvent('presented',{uid:x.uid,cardId:x.cardId,index:i,promptSize:b.prompt.length}));
    for(const x of b.prompt){const pc=card(x);if(pc.presentedBlock){b.player.block+=pc.presentedBlock;log(`《${pc.name}》提示中：防御 +${pc.presentedBlock}`);emitEvent('whilePresented',{uid:x.uid,cardId:x.cardId,effect:'block',amount:pc.presentedBlock});}if(pc.presentedSingleHitBuff)b.player.nextSingleHitBuff=Math.max(b.player.nextSingleHitBuff||0,pc.presentedSingleHitBuff);if(pc.presentedBlockIfStatus&&b.enemy.status[pc.presentedBlockIfStatus]>0){b.player.block+=1;log(`《${pc.name}》提示中：防御 +1`);emitEvent('whilePresented',{uid:x.uid,cardId:x.cardId,effect:'conditionalBlock',amount:1});}}
    emit();
  }
  ...
  function selectedMultiplier(index,instance){
    ...
    if(b.player.nextBuff)m*=1+b.player.nextBuff;if(c.selfDamage&&(b.player.nextSelfDamageBuff||0)>0)m*=1+b.player.nextSelfDamageBuff;
    ...
    if(state().character==='risk'){const h=b.hpHistory||{};if(styleIs('risk_crisis')){m*=b.player.hp<=b.player.maxHp/2?1.12:.96;if(c.selfDamage)m*=1+Math.min(.15,.05*(h.halfCrossings||0));}else if(c.selfDamage){m*=1.10+Math.min(.12,.04*(h.directionSwitches||0));if((h.healCount||0)>0)m*=1.08;}}if(c.selfDamage&&(b.player.nextSelfDamageBuff||0)>0)m*=1+b.player.nextSelfDamageBuff;
    ...
    // 章ボスはその章で解放した仕組みを理解して使うほど有利になる。
    // 未使用でも完全無効にはせず、旧章装備だけでの力押しには適応補正を掛ける。
    if(b.bossId==='boss2'){const chapterTool=tags.includes('提示操作')||!!tune||instance.wasReserved;m*=chapterTool?1.10:.68;}
    if(b.bossId==='boss3'){const chapterTool=linkActive||pairIncludesCurrent(instance)||!!conv;m*=linkActive?1.12:(chapterTool ? .90 : .64);}
    if(b.bossId==='boss4'){const doctrine=doctrineActive(),integrated=doctrine&&(!!tune||!!conv||linkActive||tags.length>=2);m*=integrated?1.12:(doctrine ? .72 : .35);}
    if(linkActive){const lm=activeLinkMode(),lh=b.linkHistory||{};m*=lm.mult||1.35;if(lm.bridge&&linkPairDisjoint())m*=1.20;/* v0.45: 連結は巨大な単発倍率ではなく、同じ組を運用し続ける熟練で少しずつ価値を得る。 */m*=1+Math.min(.08,Math.max(0,(lh.pairStreak||0)-1)*.02)+Math.min(.06,(lh.directionAlternations||0)*.02);}
    ...
    if(tune==='focus')m*=pos==='center'?1.12:.92;
    if(tune==='reserve')m*=instance.wasReserved?1.12:.94;
    if(tune==='relay')m*=linkActive?1.12:.92;
    ...
    for(const rid of state().relics||[]){const clauses=D.V34_RELIC_RULES?.[rid];if(clauses)m=v19ApplyClauses(m,clauses,v19ctx);}
    const card35=D.V35_CARD_RULES?.[instance.cardId];if(card35)m=v19ApplyClauses(m,card35,v19ctx);
    for(const rid of state().relics||[]){const clauses=D.V35_RELIC_RULES?.[rid];if(clauses)m=v19ApplyClauses(m,clauses,v19ctx);}
    const v35style=activeStyle(),v35clauses=(v35style&&D.V35_STYLE_RULES?.[v35style.id])||(!v35style&&D.V35_CHARACTER_RULES?.[state().character]);
    if(v35clauses){for(const clause of v35clauses||[]){if(v19RuleMatch(clause.when,v19ctx)){m*=clause.mult||1;if(v35style)b.v35StyleTriggerCount=(b.v35StyleTriggerCount||0)+1;else b.v35CharacterTriggerCount=(b.v35CharacterTriggerCount||0)+1;}}}
    ...
    const v47style=activeStyle();const v47charClauses=(v47style&&D.V47_STYLE_RULES?.[v47style.id])||(!v47style&&D.V47_CHARACTER_RULES?.[state().character]);if(v47charClauses)m=v19ApplyClauses(m,v47charClauses,v19ctx);
    const v53style=activeStyle();const v53clauses=(v53style&&D.V53_STYLE_RULES?.[v53style.id])||(!v53style&&D.V53_CHARACTER_RULES?.[state().character]);if(v53clauses)m=v19ApplyClauses(m,v53clauses,v19ctx);
    ...
    if(state().character==='keeper'&&instance.wasReserved){const held=Math.min(3,Number(instance.history?.heldCount||instance.history?.reserveCount||0));m*=1+held*.02;}
    if(state().character==='breaker'&&Math.max(c.hits||((c.damage!=null)?1:0),0)===1&&c.damage!=null){b.v47BreakerTriggerCount=(b.v47BreakerTriggerCount||0)+1;}
    const baseHits=Math.max(c.hits||((c.damage!=null)?1:0),0);if(baseHits===1&&c.damage!=null&&b.player.nextSingleHitBuff)m*=1+b.player.nextSingleHitBuff;
    if(b.player.nextPenalty)m*=Math.max(0,1-b.player.nextPenalty);
    return m;
  }

  function resolvedHits(instance,position){const b=Battle.current,c=card(instance),conv=conversionFor(instance),linkActive=linkedCombo(instance);let hits=c.hits||((c.damage!=null||c.kind==='hybrid')?1:0);if(c.conditionalHits&&hasAnyStatus())hits=c.conditionalHits;if(c.prevAttackHits&&b.lastWasAttack)hits=c.prevAttackHits;if(c.prevMultiHits&&b.lastWasMulti)hits=c.prevMultiHits;if(c.lowHpHits&&b.player.hp<=b.player.maxHp/2)hits=c.lowHpHits;if(c.hitsIfHpDirectionSwitches&&((b.hpHistory?.directionSwitches||0)>=c.hitsIfHpDirectionSwitches.min))hits=c.hitsIfHpDirectionSwitches.value;if(c.hitsIfRecentReshuffle&&b.recentReshuffle)hits=c.hitsIfRecentReshuffle;if(c.hitsIfPosition&&(position||b.lastChoicePosition)===c.hitsIfPosition.position)hits=c.hitsIfPosition.value;if(c.hitsIfTuned&&tuningFor(instance))hits=c.hitsIfTuned;if(c.hitsIfLinkedCombo&&linkActive)hits=c.hitsIfLinkedCombo;if(c.hitsIfLinkForward&&linkActive&&linkDirection(instance)==='forward')hits=c.hitsIfLinkForward;if(c.hitsIfLinkReverse&&linkActive&&linkDirection(instance)==='reverse')hits=c.hitsIfLinkReverse;if(c.hitsIfDeckSizeMax&&state().deck.length<=c.hitsIfDeckSizeMax.max)hits=c.hitsIfDeckSizeMax.value;if(c.hitsIfDeckSizeMin&&state().deck.length>=c.hitsIfDeckSizeMin.min)hits=c.hitsIfDeckSizeMin.value;if(c.hitsIfUniqueCopy&&deckCopies(instance.cardId)===1)hits=c.hitsIfUniqueCopy;if(c.hitsIfDuplicateCopy&&deckCopies(instance.cardId)>=2)hits=c.hitsIfDuplicateCopy;if(c.hitsIfDoctrine&&doctrineActive())hits=c.hitsIfDoctrine;if(c.hitsIfDeckTagMin&&distinctDeckTags()>=c.hitsIfDeckTagMin.min)hits=c.hitsIfDeckTagMin.value;if(c.hitsIfConverted&&conv)hits=c.hitsIfConverted;if(c.hitsIfEnemyBehavior&&enemyBehaviorCount()>0)hits=c.hitsIfEnemyBehavior;if(c.hitsIfPromptMin&&b.lastPromptCount>=c.hitsIfPromptMin.min)hits=c.hitsIfPromptMin.value;return hits;}
  function play(index){
    ...
    const linkActive=linkedCombo(selected),mult=selectedMultiplier(index,selected)*(1+(BL.Alchemy?.beforeCard(b,alchemyCard,alchemyAPI)||0)),kindKey=(conv==='bulwark'||conv==='counter_role')?'hybrid':(c.kind==='hybrid'?'hybrid':(c.kind||'utility')),boss4Layer=linkActive?'link':(tune&&conv?'dual':tune?'tune':conv?'conversion':(cardTags(selected).length>=2?'hybrid':'base'));b.player.nextBuff=0;if(c.selfDamage)b.player.nextSelfDamageBuff=0;if(Math.max(c.hits||((c.damage!=null)?1:0),0)===1&&c.damage!=null)b.player.nextSingleHitBuff=0;b.player.nextPenalty=0;b.firstCardAfterReshuffle=false;b.previousChoicePosition=prevPos;b.repeatedPosition=!!(prevPos&&prevPos===pos);b.lastChoicePosition=pos;b.positionStreak=(prevPos&&prevPos!==pos)?Math.min(2,(b.positionStreak||0)+1):0;const ph=b.positionHistory||(b.positionHistory={centerChain:0,sameChain:0,changes:0,sideAlternations:0,lastSide:null});ph.centerChain=pos==='center'?Math.min(4,(ph.centerChain||0)+1):0;ph.sameChain=(prevPos&&prevPos===pos)?Math.min(4,(ph.sameChain||0)+1):1;if(prevPos&&prevPos!==pos)ph.changes=(ph.changes||0)+1;if(pos!=='center'){if(ph.lastSide&&ph.lastSide!==pos)ph.sideAlternations=(ph.sideAlternations||0)+1;ph.lastSide=pos;}b.boss3RepeatedKind=!!(b.bossId==='boss3'&&b.lastUsedKind&&b.lastUsedKind===kindKey);b.lastUsedKind=kindKey;b.boss4RepeatedLayer=!!(b.bossId==='boss4'&&b.boss4LastLayer&&b.boss4LastLayer===boss4Layer);b.boss4LastLayer=boss4Layer;
    // prompt所有権を先に解消。選択/非選択も共通イベントとして記録する。
    emitEvent('selected',{uid:selected.uid,cardId:selected.cardId,index,position:pos});
    others.forEach(x=>{emitEvent('notChosen',{uid:x.uid,cardId:x.cardId,selectedUid:selected.uid,selectedCardId:selected.cardId});const nc=card(x);if(nc.onNotChosen?.block){b.player.block+=nc.onNotChosen.block;log(`《${nc.name}》非選択：防御 +${nc.onNotChosen.block}`);}if(nc.onNotChosen?.status)applyStatus(nc.onNotChosen.status,nc.onNotChosen.amount||1);});
    ...
    for(let r=0;r<repeat;r++){applyCard(selected,mult);if(checkEnd())return;}if(D.noteMultiStageUse)D.noteMultiStageUse(selected,b);emitEvent('used',{uid:selected.uid,cardId:selected.cardId,position:pos,hits:resolvedHits(selected,pos)});
    // v0.53: 新ビルドは巨大倍率ではなく、各メカニクスの運用を小さな横利益へ変える。
    const tags53=cardTags(selected);
    if(state().character==='vanisher'&&tags53.includes('消失')){b.player.block+=2;log('ヴァニッシャー：消失準備で防御 +2');}
    if(state().character==='scribe'&&tags53.includes('記録')){b.player.block+=2;log('スクライブ：記録整理で防御 +2');}
    if(state().character==='risk'&&c.selfDamage){b.player.block+=2;log('リスク：自傷を踏み込みに変え、防御 +2');}if(state().character==='reverser'&&c.selfDamage){b.player.nextBuff=Math.max(b.player.nextBuff,.06);log('リバーサー：代償を次の行動へ変換。');}
    if(state().character==='morph'&&((b.events?.counts?.transformed||0)>(b.v53SeenTransforms||0))){b.v53SeenTransforms=b.events.counts.transformed;b.player.block+=3;log('モーフ：変質を観測して防御 +3');}
    ...
    // v0.46: 調律は巨大倍率ではなく、条件を繰り返し成立させた履歴を小さな運用利益へ変える。
    if(tune){const th=b.tuningHistory||(b.tuningHistory={focus:0,reserve:0,relay:0});if(tune==='focus'&&pos==='center'){th.focus=Math.min(4,(th.focus||0)+1);b.player.block+=th.focus;log(`焦点調律：焦点精度 ${th.focus}／防御 +${th.focus}`);}else if(tune==='reserve'&&selectedWasReserved){th.reserve=Math.min(4,(th.reserve||0)+1);const bonus=Math.min(.08,th.reserve*.02);b.player.nextBuff=Math.max(b.player.nextBuff,bonus);log(`保留調律：保留習熟 ${th.reserve}／次カード +${Math.round(bonus*100)}%`);}else if(tune==='relay'&&linkActive){th.relay=Math.min(4,(th.relay||0)+1);b.player.block+=th.relay;log(`連結調律：接続校正 ${th.relay}／防御 +${th.relay}`);}}
    // 役割変換は同じ変換を連打するより、2枠を切り替えると小さな適応防御を得る。
    if(conv){const ch=b.conversionHistory||(b.conversionHistory={uses:0,switches:0,last:null});ch.uses++;if(ch.last&&ch.last!==conv){ch.switches++;const guard=Math.min(4,1+ch.switches);b.player.block+=guard;log(`役割変換：役割切替 ${ch.switches}／適応防御 +${guard}`);}ch.last=conv;}
    if(linkActive){const dir=linkDirection(selected),lh=b.linkHistory||(b.linkHistory={pairStreak:0,directionAlternations:0,lastDirection:null});b.linkComboCount++;lh.pairStreak=Math.min(5,(lh.pairStreak||0)+1);if(lh.lastDirection&&dir&&lh.lastDirection!==dir)lh.directionAlternations=(lh.directionAlternations||0)+1;if(dir)lh.lastDirection=dir;if(dir==='forward')b.linkForwardCount++;if(dir==='reverse')b.linkReverseCount++;log(`カード連結：${activeLinkMode().name}コンボ成立${dir==='forward'?'（A→B）':dir==='reverse'?'（B→A）':''}${lh.pairStreak>=2?'／接続熟練'+lh.pairStreak:''}。`);if(state().character==='relay'&&!styleIs('relay_direction')){b.player.block+=3;log('リレー：防御 +3');}if(hasRelic('relay_buffer')){b.player.block+=4;log('継電バッファ：防御 +4');}if(hasRelic('twin_guard')&&(c.kind==='block'||c.kind==='hybrid')){b.player.block+=4;log('双極装甲：防御 +4');}if(hasRelic('pair_memory'))b.player.nextBuff=Math.max(b.player.nextBuff,.10);if(hasRelic('chain_reserve')&&selectedWasReserved)b.player.nextBuff=Math.max(b.player.nextBuff,.20);if(activeProtocol('link_guardian')){b.player.block+=3;log('連結防護規格：防御 +3');}}
    ...
    pileFindAndRemove(selected.uid);selected.wasReserved=false;const reserveByV54=(c.reserveSelfOnPrevAttack&&b.lastWasAttack)||(c.reserveSelfOnLowDraw&&b.draw.length<=2)||(c.reserveSelfOnReshuffle&&b.recentReshuffle);if(c.reserveSelfAfterUse||reserveByV54){selected.wasReserved=true;b.reserved.push(selected);emitEvent('reserved',{uid:selected.uid,cardId:selected.cardId,self:true});log(`《${c.name}》を次回提示へ予約。`);}else if(c.vanishAfterUse){b.vanished.push(selected);emitEvent('vanished',{uid:selected.uid,cardId:selected.cardId});log(`《${c.name}》は消失した。`);}else if(tune==='overload'||c.v22Route==='exclude'||((c.selfDamage||conv==='blood_role')&&hasRelic('blood_key')))b.excluded.push(selected);else if(c.returnBottom||c.v22Route==='bottom'||tune==='recycle'||conv==='cycle_role'||(hasRelic('cycle_reserve')&&selectedWasReserved))b.draw.push(selected);else b.discard.push(selected);
    BL.Alchemy?.afterCard(b,alchemyCard,alchemyAPI);if(checkEnd())return;
    b.recentReshuffle=false;
    enemyTurn();
  }

  function reserveExisting(instance,center=false){const moved=moveTo(instance.uid,center?'centerReserved':'reserved');if(moved){moved.wasReserved=true;emitEvent('reserved',{uid:moved.uid,cardId:moved.cardId,center});log(`《${card(moved).name}》を${center?'中央に':'次ターンへ'}予約。`);}}
  ...
  function applyCard(instance,baseMult=1){
    ...
    if(c.selfDamage){const d=Math.round(c.selfDamage);recordHpChange(-d,'self');log(`自傷 ${d}`);if(hasRelic('clotting_unit')){b.player.block+=4;log('血液凝固器：防御 +4');}if(hasRelic('pain_converter')){b.player.block+=d;log(`疼痛変換器：防御 +${d}`);}if(hasRelic('red_gear'))b.player.nextBuff=Math.max(b.player.nextBuff,.15);if(hasRelic('blood_iron_reactor')){b.player.block+=3;b.player.nextBuff=Math.max(b.player.nextBuff,.1);log('鉄血炉：防御 +3 / 次カード強化');}if(checkEnd())return;}
    if(c.block!=null||c.kind==='hybrid'||conv==='bulwark'||conv==='counter_role'){
      let v=c.block||0;if(conv==='bulwark')v+=hasRelic('bulwark_joint')?8:6;if(conv==='counter_role')v*=.80;if(c.lowHpBlock&&b.player.hp<=b.player.maxHp/2)v=c.lowHpBlock;if(c.discardBlockBoost&&b.discardedEver[instance.uid])v=c.discardBlockBoost;if(c.blockIfRecentReshuffle&&b.recentReshuffle)v=c.blockIfRecentReshuffle;if(c.blockIfRecovered&&(b.hpHistory?.healCount||0)>0)v=c.blockIfRecovered;if(c.blockIfHpRecovery&&(b.hpHistory?.recoveries||0)>0)v=c.blockIfHpRecovery;if(c.blockIfLastHpDirectionUp&&b.hpHistory?.lastDirection==='up')v=c.blockIfLastHpDirectionUp;if(c.blockIfPrevAttack&&b.lastWasAttack)v=c.blockIfPrevAttack;if(c.blockIfPrevBlock&&b.lastWasBlock)v=c.blockIfPrevBlock;if(c.blockIfTurnMin&&b.turn>=c.blockIfTurnMin.turn)v=c.blockIfTurnMin.value;if(c.blockPerReshuffle)v+=c.blockPerReshuffle*b.reshuffles;if(c.blockPerDiscardedThisTurn)v+=c.blockPerDiscardedThisTurn*b.discardedThisTurn;if(c.blockPerStatusType)v+=c.blockPerStatusType*statusCount();if(c.blockIfPrevMulti&&b.lastWasMulti)v=c.blockIfPrevMulti;if(c.blockIfDrawMax&&b.draw.length<=c.blockIfDrawMax.max)v=c.blockIfDrawMax.value;if(c.blockIfPromptMax&&b.lastPromptCount<=c.blockIfPromptMax.max)v=c.blockIfPromptMax.value;if(c.blockIfPosition&&b.lastChoicePosition===c.blockIfPosition.position)v=c.blockIfPosition.value;if(c.blockIfEnemyResistMin&&b.enemy.resist>=c.blockIfEnemyResistMin.min)v=c.blockIfEnemyResistMin.value;if(c.blockIfEnemyRegenMin&&b.enemy.regen>=c.blockIfEnemyRegenMin.min)v=c.blockIfEnemyRegenMin.value;if(c.blockIfEnemyAtkMin&&b.enemy.atk>=c.blockIfEnemyAtkMin.min)v=c.blockIfEnemyAtkMin.value;if(c.blockIfTuned&&tuningFor(instance))v=c.blockIfTuned;if(c.blockIfReserved&&instance.wasReserved)v=c.blockIfReserved;if(c.blockIfLinkedCombo&&linkActive)v=c.blockIfLinkedCombo;if(c.blockIfLinkForward&&linkActive&&linkDirection(instance)==='forward')v=c.blockIfLinkForward;if(c.blockIfLinkReverse&&linkActive&&linkDirection(instance)==='reverse')v=c.blockIfLinkReverse;if(c.blockIfLinkBridge&&linkActive&&activeLinkMode().id==='bridge')v=c.blockIfLinkBridge;if(c.blockIfAdaptiveEnemy&&b.enemy.regen>0&&b.enemy.resist>=30)v=c.blockIfAdaptiveEnemy;if(c.blockIfDeckSizeMax&&state().deck.length<=c.blockIfDeckSizeMax.max)v=c.blockIfDeckSizeMax.value;if(c.blockIfDeckSizeMin&&state().deck.length>=c.blockIfDeckSizeMin.min)v=c.blockIfDeckSizeMin.value;if(c.blockIfUniqueCopy&&deckCopies(instance.cardId)===1)v=c.blockIfUniqueCopy;if(c.blockIfDuplicateCopy&&deckCopies(instance.cardId)>=2)v=c.blockIfDuplicateCopy;if(c.blockIfDoctrine&&doctrineActive())v=c.blockIfDoctrine;if(c.blockIfDeckTagMin&&distinctDeckTags()>=c.blockIfDeckTagMin.min)v=c.blockIfDeckTagMin.value;if(c.blockPerDistinctDeckTag)v+=Math.min(c.blockPerDistinctDeckTag.max||999,distinctDeckTags()*c.blockPerDistinctDeckTag.value);if(c.blockPerExcluded)v+=c.blockPerExcluded*b.excluded.length;if(c.blockIfConverted&&conv)v=c.blockIfConverted;if(c.blockIfStyle&&hasAltStyle())v=c.blockIfStyle;if(c.blockIfEnemyBurn&&b.enemy.status.burn>0)v=c.blockIfEnemyBurn;if(c.blockIfEnemyBehavior&&enemyBehaviorCount()>0)v=c.blockIfEnemyBehavior;if(c.blockPerEnemyBehavior){const cfg=c.blockPerEnemyBehavior;v+=Math.min(cfg.max||999,enemyBehaviorCount()*cfg.value);}if(c.blockPerCenterChain){const cfg=c.blockPerCenterChain;v+=Math.min(cfg.max||999,Math.max(0,(b.positionHistory?.centerChain||0)-1)*cfg.value);}if(c.blockPerPositionChange){const cfg=c.blockPerPositionChange;v+=Math.min(cfg.max||999,(b.positionHistory?.changes||0)*cfg.value);}if(c.blockPerSideAlternation){const cfg=c.blockPerSideAlternation;v+=Math.min(cfg.max||999,(b.positionHistory?.sideAlternations||0)*cfg.value);}if(c.blockPerSamePosition){const cfg=c.blockPerSamePosition;v+=Math.min(cfg.max||999,Math.max(0,(b.positionHistory?.sameChain||0)-1)*cfg.value);}if(hasRelic('steadfast_frame')&&b.turn>=4)v+=4;if(hasRelic('burnt_bandage')&&b.player.hp<=b.player.maxHp/2)v+=3;if(b.player.nextBlockFlat){v+=b.player.nextBlockFlat;b.player.nextBlockFlat=0;}v=Math.round(v*mult);b.player.block+=v;if(c.stableGuard)b.stableGuardBonus=Math.max(b.stableGuardBonus||0,2);log(`防御 +${v}`);if(c.counter||conv==='counter_role'){let cv=(c.counterIfStatus&&hasAnyStatus())?c.counterIfStatus:(c.counter||0);if(c.counterIfPosition&&b.lastChoicePosition===c.counterIfPosition.position)cv=c.counterIfPosition.value;if(c.counterIfLinkedCombo&&linkActive)cv=c.counterIfLinkedCombo;if(c.counterIfRecentReshuffle&&b.recentReshuffle)cv=c.counterIfRecentReshuffle;if(c.counterIfEnemyBehavior&&enemyBehaviorCount()>0)cv=c.counterIfEnemyBehavior;if(conv==='counter_role')cv=Math.max(cv,hasRelic('counter_hinge')?.80:.65);b.counter=Math.max(b.counter,cv);}if(c.fixedCounter){let fv=c.fixedCounter;if(c.fixedCounterIfEnemyAtkMin&&b.enemy.atk>=c.fixedCounterIfEnemyAtkMin.min)fv=c.fixedCounterIfEnemyAtkMin.value;if(c.fixedCounterIfHpRecovery&&(b.hpHistory?.recoveries||0)>0)fv=c.fixedCounterIfHpRecovery;b.fixedCounter=Math.max(b.fixedCounter,fv);}
    }
    if(c.heal)healPlayer(Math.round(c.heal*mult));if(c.healIfReshuffle&&b.reshuffles>0)healPlayer(Math.round(c.healIfReshuffle*mult));if(c.healIfLowHp&&b.player.hp<=b.player.maxHp/2)healPlayer(Math.round(c.healIfLowHp*mult));if(c.healIfEnemyRegen&&b.enemy.regen>0)healPlayer(Math.round(c.healIfEnemyRegen*mult));if(c.healIfLinkedCombo&&linkActive)healPlayer(Math.round(c.healIfLinkedCombo*mult));if(c.healIfEnemyBehavior&&enemyBehaviorCount()>0)healPlayer(Math.round(c.healIfEnemyBehavior*mult));if(c.buffNextSelfDamageAfterHeal&&(b.hpHistory?.selfDamageCount||0)>0&&b.hpHistory?.lastDirection==='up')b.player.nextSelfDamageBuff=Math.max(b.player.nextSelfDamageBuff||0,c.buffNextSelfDamageAfterHeal);if(c.buffNextSelfDamage)b.player.nextSelfDamageBuff=Math.max(b.player.nextSelfDamageBuff||0,c.buffNextSelfDamage);
    if(c.selfDamage&&instance.cardId==='blood_contract'){const v=(c.buffNextIfRecovered&&(b.hpHistory?.healCount||0)>0)?c.buffNextIfRecovered:c.buffNext;b.player.nextBuff=Math.max(b.player.nextBuff,v);return;}
    ...
    if(c.status){const statusBefore=b.enemy.status[c.status.type]||0;let amount=c.status.amount;if(c.statusIfNotChosen&&eventCount('notChosen',instance.uid)>=c.statusIfNotChosen.min)amount=c.statusIfNotChosen.amount;if(c.statusIfDeckSizeMax&&state().deck.length<=c.statusIfDeckSizeMax.max)amount=c.statusIfDeckSizeMax.amount;if(c.statusIfDeckSizeMin&&state().deck.length>=c.statusIfDeckSizeMin.min)amount=c.statusIfDeckSizeMin.amount;if(c.statusIfUniqueCopy&&deckCopies(instance.cardId)===1)amount=c.statusIfUniqueCopy;if(c.statusIfDuplicateCopy&&deckCopies(instance.cardId)>=2)amount=c.statusIfDuplicateCopy;if(c.statusIfDoctrine&&doctrineActive())amount=c.statusIfDoctrine;if(c.reshuffleStatus&&b.reshuffles>0)amount=c.reshuffleStatus;if(c.statusIfRecentReshuffle&&b.recentReshuffle)amount=c.statusIfRecentReshuffle;if(c.statusPerReshuffle)amount+=c.statusPerReshuffle*b.reshuffles;if(c.statusIfEnemyHpAbove&&b.enemy.hp>b.enemy.maxHp*c.statusIfEnemyHpAbove.ratio)amount=c.statusIfEnemyHpAbove.amount;if(c.statusIfPosition&&b.lastChoicePosition===c.statusIfPosition.position)amount=c.statusIfPosition.amount;if(c.statusIfTuned&&tuningFor(instance))amount=c.statusIfTuned;if(c.statusIfLinkedCombo&&linkActive)amount=c.statusIfLinkedCombo;if(c.statusIfAdaptiveEnemy&&b.enemy.regen>0&&b.enemy.resist>=30)amount=c.statusIfAdaptiveEnemy;if(c.statusIfConverted&&conv)amount=c.statusIfConverted;if(c.statusIfEnemyBehavior&&enemyBehaviorCount()>0)amount=c.statusIfEnemyBehavior;if(c.statusIfHpDirectionSwitches&&((b.hpHistory?.directionSwitches||0)>=c.statusIfHpDirectionSwitches.min))amount=c.statusIfHpDirectionSwitches.amount;if(b.player.nextStatusFlat){amount+=b.player.nextStatusFlat;b.player.nextStatusFlat=0;}if(conv==='detonate_role')amount*=.70;if(linkActive&&hasRelic('twin_needle'))amount+=2;applyStatus(c.status.type,Math.max(1,Math.round(amount*mult)),c.ignoreStatusResist||(c.ignoreStatusResistIfEnemyBehavior&&enemyBehaviorCount()>0));if(c.status.type==='burn'&&statusBefore>0&&c.burnRefresh){b.enemy.status.burn+=c.burnRefresh;log(`再点火：火勢 +${c.burnRefresh}`);}if(c.extraStatusIfEnemyResistMin&&b.enemy.resist>=c.extraStatusIfEnemyResistMin.min)applyStatus(c.extraStatusIfEnemyResistMin.type,c.extraStatusIfEnemyResistMin.amount,c.ignoreStatusResist);}
    if(c.statusIfEnemyBehavior&&c.statusIfEnemyBehavior.type&&enemyBehaviorCount()>0)applyStatus(c.statusIfEnemyBehavior.type,c.statusIfEnemyBehavior.amount||1);if(c.statuses)c.statuses.forEach(st=>{let amt=c.statusesIfDeckTagMin&&distinctDeckTags()>=c.statusesIfDeckTagMin.min?c.statusesIfDeckTagMin.amount:st.amount;if(c.statusesIfEnemyBehavior&&enemyBehaviorCount()>0)amt=c.statusesIfEnemyBehavior;if(conv==='detonate_role')amt*=.70;applyStatus(st.type,Math.max(1,Math.round(amt*mult)),c.ignoreStatusResist||(c.ignoreStatusResistIfEnemyBehavior&&enemyBehaviorCount()>0));});
    ...
    if(c.statusPreserveOnUse){const types=['poison','burn','bleed','frost'],type=types.sort((a,z)=>(b.enemy.status[z]||0)-(b.enemy.status[a]||0))[0];if((b.enemy.status[type]||0)>0){b.enemy.status[type]+=c.statusPreserveOnUse;log(`症状維持：${({poison:'毒',burn:'火傷',bleed:'出血',frost:'凍傷'})[type]} +${c.statusPreserveOnUse}`);}}
    ...
    if(c.buffNext)b.player.nextBuff=Math.max(b.player.nextBuff,(c.buffNextIfReshuffle&&b.reshuffles>0)?c.buffNextIfReshuffle:c.buffNext);if(c.nextBuffAfterUse)b.player.nextBuff=Math.max(b.player.nextBuff,c.nextBuffAfterUse);if(c.primeSingleHitBuff)b.player.nextSingleHitBuff=Math.max(b.player.nextSingleHitBuff||0,c.primeSingleHitBuff);if(c.nextBlockFlat)b.player.nextBlockFlat=Math.max(b.player.nextBlockFlat||0,c.nextBlockFlat);if(c.nextStatusFlat)b.player.nextStatusFlat=Math.max(b.player.nextStatusFlat||0,c.nextStatusFlat);if(c.nextSelfDamageBuffAfterMulti&&hits>=3)b.player.nextSelfDamageBuff=Math.max(b.player.nextSelfDamageBuff||0,c.nextSelfDamageBuffAfterMulti);if(c.poisonAnchor)b.poisonAnchored=true;
  ...
  function applyStatus(type,amount,ignoreResist=false){const b=Battle.current,before=statusCount(),otherBefore={poison:b.enemy.status.poison,burn:b.enemy.status.burn};if(activeProtocol('ailment_catalyst'))amount+=1;let resist=ignoreResist?0:b.enemy.resist;if(!ignoreResist&&hasRelic('sterile_needle'))resist=Math.max(0,resist-20);if(!ignoreResist&&activeProtocol('anti_resist'))resist=Math.max(0,resist-15);amount=Math.max(0,Math.round(amount*(1-resist/100)));if(amount<=0){log('状態異常は耐性で無効化された。');return;}b.enemy.status[type]+=amount;if(hasRelic('ailment_shell')&&b.statusBlockGained<6){b.player.block+=1;b.statusBlockGained++;log('症状外殻：防御 +1');}log(`${({poison:'毒',burn:'火傷',bleed:'出血',frost:'凍傷',vulnerable:'脆弱',weak:'弱体'})[type]} +${amount}`);emitEvent('statusApplied',{status:type,amount});if(state().character==='catalyst'&&!styleIs('catalyst_spectrum')&&statusCount()>before){b.player.nextBuff=Math.max(b.player.nextBuff,.10);log('カタリスト：新しい症状を観測し、次カードを強化。');}if(hasRelic('mixed_vial')&&statusCount()>before&&before>0)dealRawEnemy(4,'混成薬瓶');if(hasRelic('toxic_ash_catalyst')){if(type==='poison'&&otherBefore.burn>0)dealRawEnemy(4,'毒火触媒');if(type==='burn'&&otherBefore.poison>0)dealRawEnemy(4,'毒火触媒');}}
  function healPlayer(n){const b=Battle.current;if(n<=0)return;const before=b.player.hp;recordHpChange(n,'heal');const actual=b.player.hp-before;if(actual>0)log(`HP ${actual} 回復`);}
  ...
  function bossIntentText(){const b=Battle.current;if(b?.bossId==='boss2')return `適応攻撃 ${enemyAttackValue()} ｜ 提示操作/調律を解析回答として使用すると有利 ｜ 同じ位置を連続選択すると攻撃+3・次ターン障壁10 ｜ 再生 ${b.enemy.regen} / 耐性 ${b.enemy.resist}%`;if(b?.bossId==='boss3')return `構築攻撃 ${enemyAttackValue()} ｜ 連結/役割変換を解析回答として使用すると有利 ｜ 同じカード種別を連続使用すると攻撃+2・次ターン障壁8 ｜ 再生 ${b.enemy.regen} / 耐性 ${b.enemy.resist}%`;if(b?.bossId==='boss4')return `統合攻撃 ${enemyAttackValue()} ｜ 構築規格＋複数システムの統合運用で有利 ｜ 同じ強化系統を連続使用すると攻撃+2・次ターン障壁9 ｜ 再生 ${b.enemy.regen} / 耐性 ${b.enemy.resist}%`;return `観測攻撃 ${enemyAttackValue()} ｜ 左：次ターン障壁7 / 中央：この攻撃+3 / 右：次カード効果-15%`;}
  ...
  function enemyTurn(){
    ...
    // 状態異常は1ターン1枚の選択テンポに合わせて役割を分離する。
    // 毒：低速だが安定した持続侵蝕。敵の行動回数では増幅しない。
    if(b.enemy.status.poison>0){const pd=Math.max(1,Math.ceil(b.enemy.status.poison*1.00));dealRawEnemy(pd,'毒');if(b.poisonAnchored){b.poisonAnchored=false;log('毒の定着：この敵ターンは毒が減衰しない。');}else b.enemy.status.poison=Math.max(0,b.enemy.status.poison-1);if(checkEnd())return;}
    // 凍傷：短命な攻撃抑制。維持しないと急速に薄れる。
    const frostPenalty=b.enemy.status.frost>0?Math.max(1,Math.ceil(b.enemy.status.frost*.40)):0;
    if(frostPenalty){b.enemyTempAtk-=frostPenalty;log(`凍傷：この敵ターンの各攻撃 -${frostPenalty}`);b.enemy.status.frost=Math.max(0,b.enemy.status.frost-1);}
    let reflectedThisTurn=false;
    for(let i=0;i<actions;i++){
      emitEvent('enemyAction',{action:i+1,total:actions});
      if(state().character==='provocateur'){const n=b.events?.counts?.enemyAction||0;if(n%2===0){b.player.block+=1;log('プロヴォーカー：敵行動の観測で防御 +1');}}
      // 出血：敵が能動的に行動するほど傷が開く。高速敵への対抗軸。
      if(b.enemy.status.bleed>0){const bd=Math.max(1,Math.ceil(b.enemy.status.bleed*.5));dealRawEnemy(bd,'出血');b.enemy.status.bleed=Math.max(0,b.enemy.status.bleed-2);if(checkEnd())return;}
      BL.Alchemy?.beforeHit(b,alchemyAPI);
      if(b.stableGuardBonus){b.player.block+=b.stableGuardBonus;log(`安定防御：防御 +${b.stableGuardBonus}`);b.stableGuardBonus=0;}
      ...
      if(blocked>=incoming&&incoming>0){if(hasRelic('perfect_wall')&&!b.perfectWallHealedThisEnemyTurn){healPlayer(2);b.perfectWallHealedThisEnemyTurn=true;}if(hasRelic('iron_heartbeat'))b.player.nextBuff=Math.max(b.player.nextBuff,.25);if(hasRelic('fortress_core'))b.player.nextBuff=Math.max(b.player.nextBuff,.30);if(hasRelic('bastion_memory')&&!b.bastionMemoryHealedThisEnemyTurn){healPlayer(3);b.bastionMemoryHealedThisEnemyTurn=true;}if(b.usedBlockUid){const used=allOwnedInstances().find(x=>x.uid===b.usedBlockUid);if(used&&card(used).returnIfFullBlock){const moved=moveTo(used.uid,'draw',true);if(moved){log(`受け流し：《${card(moved).name}》を山札へ戻した。`);if(card(moved).nextBlockFlatOnReturn)b.player.nextBlockFlat=Math.max(b.player.nextBlockFlat||0,card(moved).nextBlockFlatOnReturn);}}}}
  ...
  function statusText(){const b=Battle.current;if(!b)return'';const s=b.enemy.status,parts=[];if(s.poison)parts.push(`毒${s.poison}`);if(s.burn)parts.push(`火傷${s.burn}`);if(s.bleed)parts.push(`出血${s.bleed}`);if(s.frost)parts.push(`凍傷${s.frost}`);if(s.vulnerable)parts.push(`脆弱${s.vulnerable}`);if(s.weak)parts.push(`弱体${s.weak}`);return parts.join(' / ')||'状態異常なし';}
  function upgradedCount(){const b=Battle.current;if(!b)return 0;return allOwnedInstances().filter(x=>x.upgraded).length;}

  Object.assign(Battle,{create,play,retire,swapWithCenter,redraw,enemyAttackValue,actionCountPreview,bossIntentText,statusText,invariant,allOwnedInstances,upgradedCount,enemyBehaviorCount,emitEvent,eventCount,recentEvents});
  BL.Battle=Battle;
})();
