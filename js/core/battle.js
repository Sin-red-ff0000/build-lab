...
(function(){
  ...
  function selectedMultiplier(index,instance){
    ...
    if(hasRelic('thin_deck_sensor')&&b.draw.length<=2)m*=1.2;
    ...
    if(hasRelic('center_compass')&&pos==='center')m*=1.15;
    ...
    if(hasRelic('residue_memory')&&b.discardedEver[instance.uid])m*=1.18;
    if(hasRelic('spill_engine'))m*=1+Math.min(.24,.06*b.discardedThisTurn);
    if(hasRelic('scar_lens')&&c.selfDamage)m*=1.18;
    if(hasRelic('brink_reactor')&&b.player.hp<=b.player.maxHp/2)m*=1.22;
    if(hasRelic('loop_core'))m*=1+Math.min(.24,.06*b.reshuffles);
    ...
    if(hasRelic('multihit_lens')&&Math.max(c.hits||0,c.conditionalHits||0,c.prevAttackHits||0,c.lowHpHits||0)>=5)m*=1.18;
    if(hasRelic('status_matrix')&&statusCount()>=2)m*=1.18;
    ...
    if(hasRelic('focal_core')&&pos==='center')m*=1.22;
  ...
  function applyCard(instance,baseMult=1){
    ...
    if(c.buffNext)b.player.nextBuff=Math.max(b.player.nextBuff,(c.buffNextIfReshuffle&&b.reshuffles>0)?c.buffNextIfReshuffle:c.buffNext);
