...
(function(){
  ...
  BL.Unlock = {
    ...
    rewardName(id){return D.CARDS[id]?.name||D.RELICS[id]?.name||D.TRAITS[id]?.name||D.PROTOCOLS?.[id]?.name||D.CHARACTERS?.[id]?.name||D.CHARACTER_STYLES?.[id]?.name||D.DOCTRINES?.[id]?.name||D.TUNINGS?.[id]?.name||D.CARD_CONVERSIONS?.[id]?.name||D.LINK_MODES?.[id]?.name||D.ENEMY_BEHAVIORS?.[id]?.name||D.SYSTEMS?.[id]?.name||id;},
