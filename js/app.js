...
(function(){
  ...
  function safeRender(name,fn){try{fn?.();return true;}catch(err){console.error(`[BUILD LAB v0.18] ${name} render failed`,err);return false;}}
  BL.UI.renderAll=function(){
    ...
    try{BL.Store.save();}catch(err){console.error('[BUILD LAB v0.18] save failed',err);}
  ...
  function bind(){
    ...
    const confirmBuildReset=()=>{if(confirm('アンロック・勝敗記録・進行状況は残したまま、キャラ・スタイル・デッキ・遺物・プロトコル・調律・役割変換・連結・構築規格・敵設定を初期状態へ戻します。よろしいですか？')){BL.Store.resetBuild();BL.Battle.current=null;BL.UI.renderAll();BL.UI.toast('現在の構成を初期状態へ戻しました');}};
    ...
    if(start)start.onclick=()=>startBattle(false);if(b1)b1.onclick=()=>startBattle('boss1');if(b2)b2.onclick=()=>startBattle('boss2');if(b3)b3.onclick=()=>startBattle('boss3');if(retire)retire.onclick=()=>BL.Battle.retire();const dismissModal=()=>document.getElementById('modal')?.classList.add('hidden');if(close)close.onclick=dismissModal;const modal=document.getElementById('modal');if(modal)modal.onclick=e=>{if(e.target===modal)dismissModal();};document.addEventListener('keydown',e=>{if(e.key==='Escape'&&!document.getElementById('modal')?.classList.contains('hidden'))dismissModal();});
