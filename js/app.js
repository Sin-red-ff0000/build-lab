...
(function(){
  ...
  function safeRender(name,fn){try{fn?.();return true;}catch(err){console.error(`[BUILD LAB v0.20] ${name} render failed`,err);return false;}}
  BL.UI.renderAll=function(){
    ...
    const jobs=[
      ['characters',BL.UI.renderCharacters],['characterStyles',BL.UI.renderCharacterStyles],['deck',BL.UI.renderDeck],['relics',BL.UI.renderRelics],['protocols',BL.UI.renderProtocols],['tunings',BL.UI.renderTunings],['conversions',BL.UI.renderConversions],['runes',BL.UI.renderRunes],['arcana',BL.UI.renderArcana],['links',BL.UI.renderLinks],['doctrines',BL.UI.renderDoctrines],['experiment',BL.UI.renderExperiment],['unlocks',BL.UI.renderUnlocks],['collection',BL.UI.renderCollection],['battle',BL.UI.renderBattle]
    ];
    let failed=0;for(const [name,fn] of jobs)if(!safeRender(name,fn))failed++;
    try{BL.Store.save();}catch(err){console.error('[BUILD LAB v0.20] save failed',err);}
  ...
  function bind(){
    ...
    const confirmBuildReset=()=>{if(confirm('アンロック・勝敗記録・進行状況は残したまま、キャラ・スタイル・デッキ・遺物・プロトコル・調律・役割変換・ルーン・アルカナ・連結・構築規格・敵設定を初期状態へ戻します。よろしいですか？')){BL.Store.resetBuild();BL.Battle.current=null;BL.UI.renderAll();BL.UI.toast('現在の構成を初期状態へ戻しました');}};
