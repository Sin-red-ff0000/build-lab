...
(function(){
  ...
  function renderArcana(){
    ...
    const q=(s.ui.arcanaSearch||'').trim().toLowerCase();const entries=Object.entries(D.ARCANA||{}).filter(([id,a])=>{const unlocked=!!s.unlockedArcana?.[id],sel=selected===id;if(q&&!`${a.name} ${a.tags.join(' ')} ${a.upright.desc} ${a.reversed.desc}`.toLowerCase().includes(q))return false;if(s.ui.arcanaState==='unlocked'&&!unlocked)return false;if(s.ui.arcanaState==='locked'&&unlocked)return false;if(s.ui.arcanaState==='selected'&&!sel)return false;return true;}).sort((a,b)=>a[1].name.localeCompare(b[1].name,'ja'));
    document.getElementById('arcanaResultCount').textContent=`${entries.length}件 / 全${Object.keys(D.ARCANA||{}).length}種`;area.innerHTML='';
    entries.forEach(([id,a])=>{const unlocked=!!s.unlockedArcana?.[id],sel=selected===id,el=document.createElement('div');el.className='relic-card arcana-card'+(sel?' selected':'')+(!unlocked?' locked':'');el.innerHTML=`<div class="title">${unlocked?a.name:'？？？'}</div><div class="tags">${a.tags.map(t=>`<span class="tag">${t}</span>`).join('')}</div><div class="arcana-side ${sel&&orientation==='upright'?'active':''}"><strong>正位置</strong><span>${unlocked?a.upright.desc:'未解放'}</span></div><div class="arcana-side ${sel&&orientation==='reversed'?'active':''}"><strong>逆位置</strong><span>${unlocked?a.reversed.desc:'未解放'}</span></div><div class="arcana-actions"><button data-side="upright" ${!unlocked?'disabled':''}>正位置で装備</button><button data-side="reversed" ${!unlocked?'disabled':''}>逆位置で装備</button></div>`;el.querySelectorAll('[data-side]').forEach(btn=>btn.onclick=e=>{e.stopPropagation();if(!unlocked)return BL.UI.toast('このアルカナはまだ未解放です');s.arcana={id,orientation:btn.dataset.side};BL.Store.save();BL.UI.renderAll();});area.appendChild(el);});
  }
